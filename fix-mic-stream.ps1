...
# Use regex to replace from "// Play audio chunk" to end of playNextChunk function
$pattern = '(?s)(\s+)// Play audio chunk from base64 PCM data.*?source\.start\(0\);\s+}'
$content = $content -replace $pattern, $newFunctions

# Save the file
$content | Set-Content $file -NoNewline

Write-Host "✅ mic-stream.html updated with jitter buffer functions"
