$file = "e:\Thiyo\Remote Access App\server-gateway\public\mic-stream.html"
...
for ($i = 0; $i < $lines.Count; $i++) {
    if ($lines[$i] -match "// Play audio chunk from base64 PCM data") {
        $startLine = $i
    }
    if ($startLine -ge 0 -and $lines[$i] -match "^\s+}\s*$" -and $i -gt $startLine + 60) {
        $endLine = $i
        break
    }
}

Write-Host "Found functions from line $startLine to $endLine"
...
$newFunctions = @(
    ...
    "        }",
    "",
    "        // Handle incoming audio frame (timestamped JSON)",
...
Write-Host "✅ Replaced lines $startLine to $endLine with jitter buffer functions"
